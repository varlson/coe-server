...
// LISTAR EDITAIS
slideRouter.get("/", (req: Request, res: Response) => {
  try {
    PostSchema.find({ postType: PostTypes.NEWS })
      .then((notices) => {
        if (notices.length)
...
// LISTAR UM EDITAL
slideRouter.get("/:id", (req: Request, res: Response) => {
  const { id } = req.params;
...
// DELETAR UM EDITAL
slideRouter.get("/delete/:id", (req: Request, res: Response) => {
  const { id } = req.params;
...
slideRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const notice = { title, edit_by, body } as IPost;
    if (file) {
